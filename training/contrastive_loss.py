...
def get_similarity_matrix_hpn_hymos(outputs, prototypes, chunk=2, multi_gpu=False):
    '''
        Compute similarity matrix
        - outputs: (B', d) tensor for B' = B * chunk
        - sim_matrix: (B', B') tensor
    ...
    outputs = torch.cat((outputs,prototypes))
...
def Supervised_NT_xent(sim_matrix, labels, temperature=0.5, chunk=2, eps=1e-8, multi_gpu=False):
    ...
    loss = torch.sum(Mask * sim_matrix) / (chunk * B)

    return loss


def Supervised_NT_xent_paco(sim_matrix, labels, n_centers, centers_weight=1, temperature=0.5, chunk=2, eps=1e-8, multi_gpu=False):
    '''
        In this version we have a PaCo-style set of parametric centers, which are appended to the 
        input samples, but should have a different weight in the loss
    '''
...
def hymos_hpn_loss(sim_matrix, labels, prototypes_labels, temperature=0.5, chunk=2, eps=1e-8, multi_gpu=False):
    '''
        Compute NT_xent loss
        - sim_matrix: (B', B') tensor for B' = B * chunk (first 2B are pos samples)
    '''
    ...
    labels = torch.cat((labels, prototypes_labels))

    logits_max, _ = torch.max(sim_matrix, dim=1, keepdim=True)
    sim_matrix = sim_matrix - logits_max.detach()

    size = sim_matrix.size(0)
    eye = torch.eye(size).to(device)  # (B', B')
    sim_matrix = torch.exp(sim_matrix / temperature) * (1 - eye)  # remove diagonal
    ...
    loss = torch.sum(Mask * sim_matrix) / size
